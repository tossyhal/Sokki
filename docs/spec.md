...
> 1. リアルタイムSegmenterの最大チャンク長を **18秒→5秒** に短縮(ライブ表示の初回遅延対策)(§5.5)
...
> 7. **§8.4 デザイントークン(確定値)** を新設。コミット2を確定トークン適用に変更
...
- SPEECH: 無音 **700ms** でチャンク確定。**最大5秒**で強制確定し、次チャンク先頭に**600msオーバーラップ**(話者が連続発話しても初回セグメント表示が最大5秒+推論時間に収まるようにするため。batch側の15秒上限とは独立)。
...
### 8.5 デザインモック参照と差分
Claude Design 製の過去モックは**初期の見た目の参照**に限り、実装上の正は §8.4 のデザイントークンと本仕様である。以下を守る:
...
- [ ] **rt強制確定(5秒)およびbatch強制カットのオーバーラップ部で、同一発話の二重セグメントが発生しない**
- [ ] 連続発話中でも、最初のセグメントが5秒+推論時間以内に表示される
...
- **バッチは15秒以下チャンク+abort callback 必須**(§1.2, §6.4)。音声全体1ジョブ禁止。リアルタイムは最大5秒チャンク(§5.5)。
- **過去デザインモックのコードは流用禁止**。§8.4 トークンで新規実装する。
